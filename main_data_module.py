...
class MainDataModule(L.LightningDataModule):
    ...
    def on_after_batch_transfer(self, batch, dataloader_idx):
        ...
        if self.trainer.training and self.train_dataset.gpu_transforms is not None:
            ...
            self.trainer.validating and self.val_datasets[dataloader_idx].gpu_transforms is not None
        ):
            batch = self.val_datasets[dataloader_idx].gpu_transforms(batch)
        elif (
            self.trainer.sanity_checking
            and self.val_datasets[dataloader_idx].gpu_transforms is not None
        ):
            batch = self.val_datasets[dataloader_idx].gpu_transforms(batch)
