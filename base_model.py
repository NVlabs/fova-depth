...
class MainModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self.log('val_loss_monitor_%d' % dataloader_idx, loss, add_dataloader_idx=False, sync_dist=True)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.network.parameters(), lr=float(self.optimizer_config['init_lr']), betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-4)
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=self.optimizer_config['mile_stones'], gamma=self.optimizer_config['gamma'], verbose=True)
