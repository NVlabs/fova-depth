...
def cli_main():
    ...
    if cli.config.type_to_test == 'train':
        ...
        raise RuntimeError('type_to_test must be train, test, or val. Got: %s' % cli.config.type_to_test)
    ...
    if cli.config.canon_type == 'erp':
        warp_to_canon = WarpToCanon((512,1024),canonical_model='erp',
                       normalize_baseline=cli.config.normalize_baseline,
                       aug_intrinsic_rotation='full')
    elif cli.config.canon_type == 'cube':
        warp_to_canon = WarpToCanon((384*6,384),canonical_model='cube',
                       normalize_baseline=cli.config.normalize_baseline,
                       aug_intrinsic_rotation='full')
    else:
    ...
    for im_num in range(image.size(0)):
        ...
        imageio.imwrite(os.path.join(out_dir, 'image_%d.jpg' % im_num), (im*255).cpu().numpy().astype(np.uint8))
    ...
    for src_num in range(1, image.size(0)):
        im_rec, _, _ = warpings.stereo_rectify(image[[0, src_num]], cam[[0, src_num]], to_world[[0, src_num]], (512,1024))
        im_rec = im_rec.permute(0,2,3,1).flatten(0,1)
        baseline = torch.norm(to_world[0,:3,3] - to_world[src_num,:3,3])
        print('baseline: ', baseline)
        imageio.imwrite(os.path.join(out_dir, 'rect_ref_src%d.jpg' % src_num), (im_rec*255).cpu().numpy().astype(np.uint8))
    ...
    for im_num in range(image.size(0)):
        ...
        imageio.imwrite(os.path.join(out_dir, 'canon_image_%d.jpg' % im_num), (im_vis*255).cpu().numpy().astype(np.uint8))
        pc = point_cloud[im_num].reshape(-1,3)
        pc_color = im.reshape(-1,3)
        mask = torch.all(torch.isfinite(pc),dim=1)
        pc = pc[mask,:]
