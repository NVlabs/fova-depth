...
class DDADTemporalDataset(TemporalDatasetBase):
    def __init__(self,dataset_path: str = '/mnt/disk1/Downloads/DDAD/ddad_resize',
                        cameras: list =['CAMERA_01', 'CAMERA_05', 'CAMERA_06', 'CAMERA_07', 'CAMERA_08', 'CAMERA_09'],
    ...
    def load_frame(self, sequence_key, frame_idx, index_in_sample):
        ...
        sparse_depth_path = os.path.join(self.dataset_path, sequence_key[0:6], 'depth', sequence_key[7:], base_name + '.npy')
        ...
        if self.mask_ego_vehical_depth:
            mask_path = os.path.join(self.dataset_path,'masks', sequence_key[7:], sequence_key[:6], 'mask.png')
            mask = imageio.imread(mask_path)[:,:,0]
            mask = cv2.resize(mask, (init_resize[1], init_resize[0]), interpolation=cv2.INTER_NEAREST)
            depth[mask > 1] = np.nan
...
def ddad_pose_to_matrix(pose):
    R = Rotation.from_quat([ pose['rotation']['qx'], pose['rotation']['qy'], pose['rotation']['qz'],pose['rotation']['qw']]).as_matrix()
...
def make_frame_list(pose_data):
    ...
    for pd in pose_data:
        ...
        out_dict[pd['id']['name']].append({'to_world': to_world, 'im_path': im_path, 'time_stamp': t})
...
def get_ddad_scene_dict(dataset_path, scene_nums):
    ...
    for scene_num in scene_nums:
        ...
        for cam_name, extrinsics, intrinsics in zip(cam_data['names'],cam_data['extrinsics'],cam_data['intrinsics']):
            if cam_name == 'LIDAR':
                continue
...
def remove_missing_in_scene_dict(dataset_path, scene_dicts):
    for scene_name, scene in scene_dicts.items():
        for cam_name, cam in scene.items():
            cam['frame_list'] = [f for f in  cam['frame_list'] if test_frame_exists(dataset_path, scene_name, cam_name, f)]
