...
class Kitti360TemporalDataset(TemporalDatasetBase):
    ...
    def __init__(
        self,
        dataset_path: str = "/home/daniel/Downloads/KITTI-360",
        ...
        cam0_to_world = np.loadtxt(
            os.path.join(dataset_path, "data_poses", scene_name, "cam0_to_world.txt")
        )
        poses = np.loadtxt(os.path.join(dataset_path, "data_poses", scene_name, "poses.txt"))
        cam_to_pose = loadCalibrationCameraToPose(
            os.path.join(dataset_path, "calibration", "calib_cam_to_pose.txt")
        )
        self.cam_to_pose = cam_to_pose[image_num]

        self.image_path = os.path.join(
            dataset_path, "data_2d_raw", scene_name, image_num, "data_rgb"
        )
        ...
        for i in range(N):
            ...
            frame = {
                "to_world": to_world.astype(np.float32),
                "image_path": "%010d.png" % cam0_to_world[i, 0],
            }
