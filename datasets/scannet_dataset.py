...
class SequenceFolder(data.Dataset):
    ...
    def crawl_folders(self):
        ...
        if os.path.exists(cache_path):
            ...
            sequence_set = []
            imgs = np.genfromtxt(os.path.join(self.root,'new_orders/'+self.ttype[:-4]+"/"+self.ttype[:-4]+'new_orders_v.list'), delimiter = ' ', dtype = 'unicode')
            ...
            for i in range(len(imgs)):
                ...
                n_img = imgs[i,1] + '.jpg'
                intrinsics = np.genfromtxt(os.path.join(self.root,scene,'intrinsic/intrinsic_depth.txt')).astype(np.float32).reshape((4, 4))[:3,:3]
                ...
                n_index = [n_img]
                sample = {'intrinsics': intrinsics, 'tgt': os.path.join(self.root,scene,'color/'+img), 'tgt_depth': depth, 'ref_imgs': [], 'pose_tgt': pose_tgt, 'pose_src': [], 'gt_nmap': gt_nmap, 'ref_depths': []}
                for j in n_index:
                    sample['ref_imgs'].append(os.path.join(self.root,scene,'color/'+j))
                    sample['ref_depths'].append(os.path.join(self.root,scene,'depth/',(j[:-4]+'.npy')))
                    sample['pose_src'].append(os.path.join(self.root,scene,'pose/',(j[:-4]+'.txt')))
    ...
    def __getitem__(self, index):
        ...
        ref_pose = np.genfromtxt(os.path.join(self.root,sample['pose_tgt'])).astype(np.float32).reshape((4, 4))
        ...
        src_depths = [np.load(os.path.join(self.root,ref_depth)) for ref_depth in sample['ref_depths']]
        src_imgs = [load_as_float(os.path.join(self.root,ref_img))/255 for ref_img in sample['ref_imgs']]
...
class ScanNetDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.init_resize is not None:
            ...
            depths = [cv2.resize(x,(self.init_resize[1],self.init_resize[0]),interpolation = cv2.INTER_NEAREST) for x in depths]

       
        intrinsics = np.copy(intrinsics)
        n_intrinsics = utils.normalized_intrinsics_from_pixel_intrinsics(torch.from_numpy(intrinsics),(480,640))
