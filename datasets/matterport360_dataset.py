            ...
            for i in range(len(names)):
                for j in range(i + 1, len(names)):
                    if dists[i][j] < dist_threshold:
                        pairs.append(
                            (os.path.join(scene_path, names[i]), os.path.join(scene_path, names[j]))
                        )
    ...
    def __getitem__(self, idx):
        ...
        for path in pair:
            ...
            if self.init_resize is not None:
                im = cv2.resize(
                    im,
                    (self.init_resize[1], self.init_resize[0]),
...
if __name__ == "__main__":
    ...
    dataset = Matterport360Dataset(
        "/media/daniel/drive3/Datasets/matterport360/data",
        "datasets/matterport_split/scenes_test.txt",
