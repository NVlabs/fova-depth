...
class RandomCropFlip(nn.Module):
    def __init__(self, crop_scale_range=None, 
                       flip_probability=0,
                       keys=['image', 'depth'],
    ...
    def get_crop_matrix(self, N, device):
        new_half_width = (self.crop_scale_range[1]-self.crop_scale_range[0])*torch.rand(N,device=device) + self.crop_scale_range[0]
        #(N,2)
    ...
    def forward(self, sample):
        ...
        cropped_list, _ = affine_transform_image(to_crop_list, crop_matrix, interp_mode=self.interp_modes)
...
class ColorJitterBatch(nn.Module):
    ...
    def __init__(self, brightness=(0.8,1.2), contrast=(0.8,1.2), saturation=(0.8,1.2), hue=(-0.1,0.1), individually=False):
        super().__init__()
        self.color_aug = torchvision.transforms.ColorJitter(brightness=brightness, contrast=contrast, saturation=saturation, hue=hue)
        self.individually = individually

    #all images in a batch get the same augmentation
    def forward(self, sample):
        for b in range(len(sample['image'])):
            params = self.color_aug.get_params(brightness=self.color_aug.brightness, contrast=self.color_aug.contrast, saturation=self.color_aug.saturation, hue=self.color_aug.hue)
...
def color_jitter_specify_params(img, fn_idx, brightness_factor, contrast_factor, saturation_factor, hue_factor):
        for fn_id in fn_idx:
