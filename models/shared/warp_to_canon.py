...
from data_processing.make_basis import get_both_orthogonal
...
class WarpToCanon(nn.Module):
    ...
    def forward(self, sample):
        ...
        if self.training and (self.aug_intrinsic_rotation == 'full'):
            R_new_to_world = So3.random(batch_size=b*M, device=device).matrix().reshape(b,M,3,3)
        else:
            R_new_to_world = to_world[:,:,:3,:3]
        
        R_old_to_world_inv = to_world[:,:,:3,:3].transpose(-1,-2)
        R = torch.bmm(R_old_to_world_inv.flatten(0,1),R_new_to_world.flatten(0,1)).unflatten(0,(b,M))

        canon_image_and_distance, _ = resample_by_intrinsics([sample['image'], distance], 
                                                          sample['camera'], 
                                                          canonical_camera, 
                                                          self.h_w, 
