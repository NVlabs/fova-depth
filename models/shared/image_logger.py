...
class ImageLogger:
    def __init__(self, log_freq=100, val_log_freq=100, distance_min_max=(0, 6), cube_mode=False):
    ...
    def __call__(self, my_model, sample, batch_idx, net_out, prefix):
        ...
        if write_to_log:
            ...
            my_model.logger.experiment.add_image(
                prefix + "_gt_distance",
                scale_between(canon_distance, self.distance_min_max),
                image_num,
            )
            ...
            my_model.logger.experiment.add_image(
                prefix + "_pred_distance",
                scale_between(pred_distance, self.distance_min_max),
                image_num,
            )
            ...
            my_model.logger.experiment.add_image(
                prefix + "_pred_refined_distance",
                scale_between(pred_refined_distance, self.distance_min_max),
                image_num,
            )
