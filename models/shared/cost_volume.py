...
class BasicWeightModule(nn.Module):
    def __init__(self, groups, hidden=16):
        ...
        self.net = nn.Sequential(nn.Linear(groups,hidden),
                                 nn.ReLU(True),
...
def cost_volume(image, cameras, to_world, hypo_depths, sphere_sweep=True):
    ...
    ref_to_src = torch.einsum('bmij,bjk->bmik',torch.inverse(src_to_worlds.float()), ref_to_world) #(b,M-1,4,4)
    pc_src = apply_affine(ref_to_src, pc_ref.unsqueeze(1).expand(-1,ref_to_src.size(1),-1,-1,-1,-1)) #(b,M-1,d,h,w,3)
    pc_src_proj,_, valid2 = src_cams.project_to_pixel(pc_src,depth_is_along_ray=sphere_sweep) #(b,M-1,d,h,w,2)
