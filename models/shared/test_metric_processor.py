...
class TestMetricProcessor(nn.Module):
    ...
    def get_stats(self, gt, pred):
        ...
        stats_dict["delta1"] = torch.nanmean(
            torch.where(torch.isfinite(thresh), (thresh < 1.25).float(), thresh), dim=1
        )
        stats_dict["delta2"] = torch.nanmean(
            torch.where(torch.isfinite(thresh), (thresh < 1.25**2).float(), thresh), dim=1
        )
        stats_dict["delta3"] = torch.nanmean(
            torch.where(torch.isfinite(thresh), (thresh < 1.25**3).float(), thresh), dim=1
        )
        ...
        stats_dict["log_rmse"] = torch.sqrt(
            torch.nanmean((torch.log(gt) - torch.log(pred)) ** 2, dim=1)
        )
        return stats_dict
    ...
    def step(self, net_out, sample, save_dir):
        ...
        if "pred_distance_original" in net_out:
            ...
            pred_refined = (
                net_out["pred_refined_distance_original"].flatten(1).clamp(*self.gt_clamp_range)
            )
        ...
        if sample["idx"][0] % self.save_freq == 0:
            ...
            pc_color = im[mask, :]
            write_ply_standard(
                os.path.join(curr_save_dir, "pred_pc.ply"),
                pc.reshape(-1, 3),
                colors=pc_color.reshape(-1, 3),
            )
            for i in range(sample["image"].size(1)):
                im = sample["image"][0, i].permute(1, 2, 0)
                imageio.imwrite(
                    os.path.join(curr_save_dir, "image_%d.jpg" % i),
            ...
            if "pred_refined_distance_original" in net_out:
                ...
                imageio.imwrite(
                    os.path.join(curr_save_dir, "log_pred_depth.png"),
                    (torch.log(pred_depth).cpu().numpy() * 50).astype(np.uint8),
                )
        return sd
