...
from models.cube.cube_conv import dense_to_sparse, sparse_to_dense, SparseCubeConv2d, SparseBilinearUpsample
...
class CubeMidasNet(nn.Module):
    ...
    def __init__(self, features=256, out_ch=1, pretrained=True, input_ch=3, model='resnet34', use_new_output=False):
        ...
        self.pretrained = MidasSparsePadFeatureExtractor(pretrained=pretrained, input_ch=input_ch, resnet_model=resnet_model)
        ...
        self.layer1_rn = SparseCubeConv2d(layer_out_channels[0], features, kernel_size=3, stride=1, padding=1, bias=False)
        self.layer2_rn = SparseCubeConv2d(layer_out_channels[1], features, kernel_size=3, stride=1, padding=1, bias=False)
        self.layer3_rn = SparseCubeConv2d(layer_out_channels[2], features, kernel_size=3, stride=1, padding=1, bias=False)
        self.layer4_rn = SparseCubeConv2d(layer_out_channels[3], features, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, sparse_cube_maps, to_process, batch_size):
        ...
        layer_1, layer_2, layer_3, layer_4 = self.pretrained(sparse_cube_maps, to_process, batch_size)
...
def basic_single_image_test():
    ...
    image0 = imageio.imread('/home/dlichy/Desktop/Project/ComparisonCode/Normal-Assisted-Stereo/scannet/val/scene0011_00/color/0000.jpg')
...
def test_on_cube_map():
    device='cuda'
    dataset = Matterport360NNDataset('/home/dlichy/Downloads/matterport360/data','datasets/matterport_split/scenes_train.txt',init_resize=(2*128,2*256))
    ...
    os.makedirs(out_dir,exist_ok=True)
    imageio.imwrite(os.path.join(out_dir,'image.jpg'), cube_image_flat[save_batch].permute(1,2,0).cpu())
    imageio.imwrite(os.path.join(out_dir,'depth.png'), cube_depth_flat[save_batch].detach().squeeze().cpu())
