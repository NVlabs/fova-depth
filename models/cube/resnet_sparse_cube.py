...
class BasicBlock(nn.Module):
    ...
    def forward(self, args) -> Tensor:
        ...
        out = self.conv1((x, to_process, batch_size))[0]
        out = self.bn1(out)
        out = self.relu(out)
        ...
        if self.downsample is not None:
            identity = self.downsample((x, to_process, batch_size))[0]

        out += identity
        out = self.relu(out)

        return out, to_process, batch_size
...
class Bottleneck(nn.Module):
    ...
    def forward(self, args) -> Tensor:
        ...
        out = self.conv3((out, to_process, batch_size))[0]
        out = self.bn3(out)
        if self.downsample is not None:
            identity = self.downsample((x, to_process, batch_size))[0]
