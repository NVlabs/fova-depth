...
class FOVADepthCube(nn.Module):
    def __init__(self, greater_than_two_images: bool = False, 
        ...
        self.prepare_data = WarpToCanon((6*cube_side,cube_side), canonical_model='cube', normalize_baseline=normalize_baseline, aug_intrinsic_rotation=aug_intrinsic_rotation)

        
        self.feature = SparsePadFeatureExtractor(pretrained=pretrained, add_mixing=add_feature_mixing)
        print('feature out channels', self.feature.out_channels)

        self.depth_net = DepthModuleDotMulti(groups=groups,greater_than_two=greater_than_two_images)
        self.refine_network = RefineNetMono(model=refine_net_backbone, pretrained=pretrained, 
                                     scale_factor=refine_scale_factor, pred_residual=refine_pred_residual)
    ...
    def forward(self, sample):
        ...
        out = self.depth_net(features, canon_camera, canon_to_world, depth_hypos, idx_to_process[:,0])
        pred_dist = out['distance'] #(b,6,1,cube_side/4,cube_side/4)
        pred_inv_dist = 1/pred_dist.detach().clamp(min=min_depth)
        
        #Refine stage
        up_pred_inv_dist = F.interpolate(pred_inv_dist.flatten(0,1),scale_factor=4, mode='nearest').unflatten(0,(-1,6))
        ...
        if self.warp_to_original_cam:
            ...
            recon_depth, _ = resample_by_intrinsics(net_out['pred_distance'], canon_camera[:,0], sample['camera'][:,0], sample['image'].shape[3:5], rotation_trg_to_src=RT, interp_mode='bilinear', depth_is_along_ray=True)
            net_out['pred_distance_original'] = recon_depth*sample['baseline_distance'].reshape(-1,1,1,1)

            recon_depth, _ = resample_by_intrinsics(net_out['pred_refined_distance'], canon_camera[:,0], sample['camera'][:,0], sample['image'].shape[3:5], rotation_trg_to_src=RT, interp_mode='bilinear', depth_is_along_ray=True)
            net_out['pred_refined_distance_original'] = recon_depth*sample['baseline_distance'].reshape(-1,1,1,1)
