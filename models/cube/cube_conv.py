...
import imageio
import numpy as np
from nvtorchcam.utils import samples_from_cubemap, get_normalized_grid_cubemap, flatten_cubemap_for_visual

def sparse_to_dense(sparse_cube,idx,batch_size):
    #(k,c,w,w) (k) int
    #return (b,6,c,w,w)
    dense_cube = torch.zeros(batch_size,6, *sparse_cube.shape[1:], dtype=sparse_cube.dtype, device=sparse_cube.device)
...
def get_padding_indices(channels, width, padding):
    ...
    device='cuda'
    pixel_cube = torch.arange(channels*width*width,device=device).unsqueeze(0).expand(6,-1).reshape(6,channels,width,width)
    #(6,width+2*padding,width+2*padding,3)
    rays = get_normalized_grid_cubemap(width, device, pad=padding).unflatten(0,(6,-1))
    pixel_idx = samples_from_cubemap(pixel_cube.transpose(0,1).flatten(1,2), rays,mode='nearest').transpose(0,1)
    pixel_idx = pixel_idx.clamp(min = 0, max= channels*width*width - 1) #why this is necessary needs more investigation
    pixel_idx = pixel_idx.long()
    #get face each index is pointing to
    face_cube = torch.arange(0,6).reshape(-1,1,1).expand(6,width,width).unsqueeze(0).flatten(1,2)
...
def get_padding_indices_pad_only(channels, width, padding):
    ...
    pixel_idx, face_idx = get_padding_indices(channels, width, padding) #(6,c,w+2p,w+2p) (6,w+2p,w+2p)
...
class SparseIdxCubePadImproved(nn.Module):
    ...
    def forward(self, cube, to_process, batch_size):
        ...
        if self.input_width != w or self.channels != c:
            ...
            pixel_idx_tb, pixel_idx_lr, face_idx_tb, face_idx_lr = get_padding_indices_pad_only(c,w,self.pad_width)
        ...
        with torch.no_grad():
            ...
            #(b,6,2p,w+2p)
            face_idx_tb = self.face_idx_tb.unsqueeze(0)+ 6*torch.arange(batch_size,device=device).reshape(-1,1,1,1)
            #(k,2p,w+2p)
            ...
            #(b,6,w,2p)
            face_idx_lr = self.face_idx_lr.unsqueeze(0)+ 6*torch.arange(batch_size,device=device).reshape(-1,1,1,1)
            #(k,w,2p)
        ...
        padded_cube = torch.cat([pad_lr[:,:,:,:self.pad_width], cube, pad_lr[:,:,:,-self.pad_width:]],dim=3)
        padded_cube = torch.cat([pad_tb[:,:,:self.pad_width,:], padded_cube, pad_tb[:,:,-self.pad_width:,:]], dim=2)
       
        return padded_cube


class SparseCubeConv2d(nn.Module):
    def __init__(self, f_in, f_out, kernel_size=3, stride=1, padding=0, bias=True,groups=1, dilation=1):
        super().__init__()
        self.padding = padding
        self.conv = nn.Conv2d(f_in,f_out,kernel_size, stride, padding=0,bias=bias,groups=groups, dilation=dilation)
        if padding != 0:
...
def test_sparse_padding(cube_map, idx_to_process = None, pad_width=16):
    ...
    padded_flat_cubemap = flatten_cubemap_for_visual(padded_cube_map.transpose(1,2).flatten(2,3)).cpu()

    save_dir = 'test_sparse_cube_padding'
    os.makedirs(save_dir,exist_ok=True)
    for i in range(batch_size):
        imageio.imwrite(os.path.join(save_dir,'original_%d.png' % i), (flat_cubemap[i]*255).permute(1,2,0).numpy().astype(np.uint8))
        imageio.imwrite(os.path.join(save_dir,'padded_%d.png' % i), (padded_flat_cubemap[i]*255).permute(1,2,0).numpy().astype(np.uint8))
